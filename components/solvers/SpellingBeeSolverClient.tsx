...
export function SpellingBeeSolverClient() {
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1fr_22rem]">
      <div className="min-w-0 rounded-2xl border border-[#E5DED3] bg-[#FFFDF9] p-5 shadow-sm">
        <div className="grid gap-4 sm:grid-cols-[10rem_1fr]">
          <label className="grid gap-2 text-sm font-bold">
            ...
            <input
              value={center}
              maxLength={1}
              onChange={(event) => setCenter(event.target.value.toUpperCase())}
              className="rounded-lg border border-[#D4CABD] bg-white px-3 py-2 text-center text-2xl font-black outline-none focus:border-[#315C4C] focus:ring-4 focus:ring-[#315C4C]/20"
            />
          </label>
          <label className="grid gap-2 text-sm font-bold">
            Six outer letters
            <input
              value={outer}
              maxLength={6}
              onChange={(event) => setOuter(event.target.value.toUpperCase())}
              placeholder="ABLTCN"
        ...
        {!ready && searched ? (
          <p className="mt-3 text-sm font-semibold text-[#A7473D]">Enter exactly one center letter and six outer letters.</p>
        ) : null}
        <div className="mt-5 flex flex-wrap gap-3">
          <button type="button" onClick={() => setSearched(true)} className="rounded-lg bg-[#315C4C] px-5 py-2.5 text-sm font-bold text-white hover:bg-[#274B3E]">
            Find words
          </button>
          <button type="button" onClick={example} className="rounded-lg border border-[#D4CABD] bg-[#EDE6DC] px-5 py-2.5 text-sm font-bold hover:bg-[#E3D9CC]">
            Load example
          </button>
          <button
            type="button"
            ...
            className="rounded-lg border border-[#D4CABD] px-5 py-2.5 text-sm font-bold hover:bg-[#EDE6DC]"
          >
            Clear
          </button>
        </div>
