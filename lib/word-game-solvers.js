...
function mergeMaxCounts(...countMaps) {
  const merged = new Map();
  for (const counts of countMaps) {
    for (const [letter, count] of counts.entries()) {
      merged.set(letter, Math.max(merged.get(letter) || 0, count));
    }
  }
  return merged;
}
...
export function solveWordle(words, options = {}) {
  ...
  const requiredCounts = mergeMaxCounts(letterCounts(includes), letterCounts(misplaced.map((item) => item.letter).join("")));
